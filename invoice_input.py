...
class SkuIdentifier(object):
  ...
  def __eq__(self, other: any) -> bool:
    ...
    return self.__sku == other.__sku and self.__program == other.__program
  

  def __hash__(self) -> int:
    return hash((self.__sku, self.__program))
...
class InvoiceEntryRow(object):
  ...
  def to_invoice_input(self, contacts: dict[str, int], companies: dict[str, int]):
    domain = COMPANY_DOMAIN_TEMPLATE.format(self.__program.lower(), str(self.__team_number))
    return InvoiceInput(
